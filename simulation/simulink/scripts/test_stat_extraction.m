% Test how to extract statistics correctly
model_name = 'RetinaGuard_SimEvents_Day2';
addpath('..\models');
load_system(model_name);

% Enable statistics logging on the model
set_param(model_name, 'SimEventsEnableStatistics', 'on');
...
% Attempt 1: Check if SimLog is generated
fprintf('=== Attempt 1: simOut.SimLog ===\n');
if isprop(simOut, 'SimLog')
    disp(simOut.SimLog);
else
    fprintf('simOut.SimLog does not exist.\n');
end

% Attempt 2: Block parameters
fprintf('\n=== Attempt 2: get_param after sim ===\n');
try
    cap_util = get_param([model_name '/Capture Server'], 'Utilization');
    fprintf('Capture Server Utilization: %s\n', mat2str(cap_util));
catch ME
