 ...
 * @property {(id: string | number) => void} [onItemClick] - Optional click handler for a card.
 */
...
const HoverRevealCards: React.FC<HoverRevealCardsProps> = ({
  ...
  return (
    ...
    <div
      role="list"
      className={cn(
        'group grid w-full max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3',
        className
      )}
    >
      {items.map((item) => (
        <div
          ...
          className={cn(
            'relative h-96 cursor-pointer overflow-hidden rounded-xl bg-cover bg-center shadow-lg transition-all duration-500 ease-in-out',
