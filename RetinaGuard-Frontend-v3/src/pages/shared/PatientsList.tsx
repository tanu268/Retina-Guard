          ...
          <motion.div variants={stagger} initial="initial" animate="animate" className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {patients.map((p) => (
              <motion.div
                ...
                className="text-left bg-white border border-slate-200 rounded-lg p-5 shadow-[0_1px_2px_0_rgb(15_23_42/0.04),0_1px_3px_0_rgb(15_23_42/0.06)] transition-shadow duration-200 hover:shadow-[0_4px_8px_-2px_rgb(15_23_42/0.05),0_12px_28px_-6px_rgb(15_23_42/0.10)] cursor-pointer group"
              >
                ...
                <dl className="grid grid-cols-2 gap-3 text-[12px] pointer-events-none">
                  <div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between pointer-events-none">
                  <span className="text-[11.5px] text-slate-400">Registered {formatRelative(p.created_at)}</span>
                  <span className="text-slate-300 transition-transform duration-300 group-hover:-translate-y-1 group-hover:translate-x-1"><IconArrowRight size={14} /></span>
                </div>
                </motion.div>
            ))}
          </motion.div>
